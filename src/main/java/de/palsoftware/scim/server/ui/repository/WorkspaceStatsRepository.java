    ...
    private static final String WORKSPACE_STATS_SQL = """
            WITH counts AS (
                SELECT
                    ...
                    (SELECT NULLIF(reltuples::bigint, 0) FROM pg_class WHERE relname = 'scim_users') AS total_users,
                    (SELECT COUNT(*) FROM scim_groups g WHERE g.workspace_id = :workspaceId) AS workspace_groups,
                    (SELECT NULLIF(reltuples::bigint, 0) FROM pg_class WHERE relname = 'scim_groups') AS total_groups,
                    (SELECT COUNT(*) FROM workspace_tokens t WHERE t.workspace_id = :workspaceId) AS workspace_tokens,
                    (SELECT NULLIF(reltuples::bigint, 0) FROM pg_class WHERE relname = 'workspace_tokens') AS total_tokens,
                    (SELECT COUNT(*) FROM scim_request_logs l WHERE l.workspace_id = :workspaceId) AS workspace_logs,
                    (SELECT NULLIF(reltuples::bigint, 0) FROM pg_class WHERE relname = 'scim_request_logs') AS total_logs,
                    ...
                    (SELECT NULLIF(reltuples::bigint, 0) FROM pg_class WHERE relname = 'scim_group_memberships') AS total_group_memberships
            )
            SELECT
                workspace_users,
                workspace_groups,
